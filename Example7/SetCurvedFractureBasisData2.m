function [NumberofFractures,NumberofElementsPassed,FracturesPath,FracturesLength,...
    ...
    SetCurvedFractureBasisData2(coordinates,elements4,quad_x)
% semicircles
...
x_partition = (3/8-1/8*cos(t_partition)).*(t_partition<=pi) + (5/8+1/8*cos(t_partition)).*(t_partition>pi);
...
x_sampling = (3/8-1/8*cos(t_sampling)).*(t_sampling<=pi)+(5/8+1/8*cos(t_sampling)).*(t_sampling>pi); 
y_sampling = 1/2+1/8*sin(t_sampling); 
...
dx_dt = 1/8*sin(t_sampling).*(t_sampling<=pi) - 1/8*sin(t_sampling).*(t_sampling>pi);
dy_dt = 1/8*cos(t_sampling);
