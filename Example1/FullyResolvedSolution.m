function FullyResolvedSolution
...
hold on;
surf(Xq,Yq,Vq)
colormap(parula);xlim([Interp_xmin,Interp_xmax]); ylim([Interp_ymin Interp_ymax]);view(30,30);%shading flat;%caxis([-100000, 1200000]); 
...
%colormap(parula);xlim([xmin,xmax]); ylim([ymin ymax]);view(30,30);%shading flat;%caxis([-100000, 1200000]); 
%contour(reshape(coordinates(:,1),Cell_N+1,Cell_M+1),reshape(coordinates(:,2),Cell_N+1,Cell_M+1),reshape(u,Cell_N+1,Cell_M+1))
%hold off;
